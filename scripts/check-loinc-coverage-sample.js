...
function tryAddCodingArray(codingArr, set) {
...
function collectCodesFromResource(resource, set) {
  ...
  if (resource.code && resource.code.coding) tryAddCodingArray(resource.code.coding, set);
  if (resource.coding) tryAddCodingArray(resource.coding, set);
  if (resource.component && Array.isArray(resource.component)) {
    resource.component.forEach(comp => {
      if (comp.code && comp.code.coding) tryAddCodingArray(comp.code.coding, set);
    });
  }
  if (resource.valueCodeableConcept && resource.valueCodeableConcept.coding) tryAddCodingArray(resource.valueCodeableConcept.coding, set);
...
(function main() {
  try {
    ...
    const resources = Array.isArray(data) ? data.slice(0, sampleSize) : [];
    ...
    console.log(`Sampled ${resources.length} resources.`);
    console.log(`Found ${total} unique coding.code values in sample.`);
    ...
    console.error('Error running sample coverage:', err);
  }
})();
